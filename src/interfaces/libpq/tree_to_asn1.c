	...
	if (ss->fromClause != NULL)
	{
		...
		while (lc != (ListCell *)NULL)
		{
			...
			if (rv->catalogname)
			{
				...
				s->choice.asciiString.buf  = (uint8_t *)rv->catalogname;
			...
			if (rv->schemaname)
			{
				...
				s->present = ASNString_PR_asciiString;
				s->choice.asciiString.buf  = (uint8_t *)rv->schemaname;
			...
			if (rv->relname)
			{
				...
				s->present = ASNString_PR_asciiString;
				s->choice.asciiString.buf  = (uint8_t *)rv->relname;
...
res_target_to_asn1(ResTarget *rt)
{
	...
	if (valNodeType == T_ColumnRef)
	{
		...
		else if (valType == T_String)
		{
			...
			columnItem->choice.str.choice.asciiString.buf  = (uint8_t *)v->val.str;
	...
	if (rt->name)
	{
		...
		columnName->choice.asciiString.buf	= (uint8_t *)rt->name;
	...
	if (nt == T_A_Const)
	{
		...
		switch(const_nt)
		{
			...
			case T_String :
				...
				expr->choice.stringConst.choice.asciiString.buf	 = (uint8_t *)ac->val.val.str;
